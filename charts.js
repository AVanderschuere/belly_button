...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // Bar 4. Create a variable that filters the samples for the object with the desired sample number.
    var samplesFilter = samples.filter(sampleObj => sampleObj.id == sample);

    //  Bar 5. Create a variable that holds the first sample in the array.
    var result = samplesFilter[0];

    // Bar 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otu_ids = result.otu_ids;
    var otu_labels = result.otu_labels;
    var sample_values = result.sample_values;

    // Bar 7. Create the yticks for the bar chart.
    // Hint: Get the the top 10 otu_ids and map them in descending order  
    //  so the otu_ids with the most bacteria are last. 

    var yticks = otu_ids.slice(0, 10).map(otuID => `OTU ${otuID}`).reverse();

    // Bar 8. Create the trace for the bar chart. 
    var barData = [{
      type: 'bar',
    ...
    // Bar 9. Create the layout for the bar chart. 
    var barLayout = {
    ...
    // Bar 10. Use Plotly to plot the data with the layout. 
    ...
    // Guage 2. Create a variable that holds the first sample in the metadata array.
    ...
    // Guage 4. Create the trace for the gauge chart.
    var gaugeData = [
      {
    ...
    // Guage 5. Create the layout for the gauge chart.
    var gaugeLayout = {
      width: 600, height: 450, margin: { t: 0, b: 0 }
    };

    // Guage 6. Use Plotly to plot the gauge data and layout.
